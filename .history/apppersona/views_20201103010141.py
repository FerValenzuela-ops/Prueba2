...
def bootstrap_form(request):
    ...
    if is_valid_queryparam(min_rut_query):
        qs = qs.filter(celular__gte=min_rut_query)

    if is_valid_queryparam(max_rut_query):
        qs = qs.filter(celular__lt=max_rut_query)    
...
def contactosPersonas(request):
    if request.method == "POST":
        contacto = ExtendedUserCreationForm(request.POST)

        if contacto.is_valid() :
            contactos = contacto.save()
    ...
    return render(request, "apppersona/contacto.html", context)       
