...
class FormularioPersona(forms.ModelForm): # Formulario para la clase persona

    class Meta:
        model = Persona
        fields = ('nombre', 'apellido', 'email', 'celular',
                  'region', 'rut', 'edad')
        widgets = { # Aqui se le asignan los atributos y estilos  a los campos que el usuario debe rellenar
        ...
        labels = { # labels para los campos del formulario
            'nombre': _('Nombre'),
            'apellido': _('Apellido'),
...
class FormularioContacto(forms.ModelForm): # Formulario para la creacion de solic
    ...
    class Meta:
        ...
        fields = ('nombre', 'asunto', 'email', 'telefono',
                  'mensaje')
        widgets = {
            'nombre': Textarea(attrs={'rows': 1, 'cols': 30, 'style': 'resize:none;', 'id': 'nombreContacto', 'class': 'form-control', 'placeholder':"Nombre"}),
            'asunto': Textarea(attrs={'rows': 1, 'cols': 30, 'style': 'resize:none;', 'id': 'asuntoContacto', 'class': "form-control", 'placeholder':"Asunto"}),
            'email': EmailInput(attrs={'rows': 1, 'cols': 30, 'style': 'resize:none;', 'id': 'emailContacto', 'class': "form-control",  'type': 'email',  'placeholder':"Email"}),
            'telefono': Textarea(attrs={'rows': 1, 'cols': 30, 'style': 'resize:none;', 'id': 'telefonoContacto', 'class': "form-control", 'placeholder':"Telefono"}),
            'mensaje' : Textarea(attrs={'rows': 1, 'cols': 30, 'style': 'resize:none;', 'id': 'mensajeContacto', 'class': 'form-control',  'placeholder':"Mensaje"}),
           }        