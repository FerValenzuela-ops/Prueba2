...
def bootstrap_form(request):
    ...
    min_rut_query = request.GET.get('min_rut_query')
    max_rut_query = request.GET.get('max_rut_query')

    if is_valid_queryparam(min_rut_query):
        qs = qs.filter(rut__gte=min_rut_query)

    if is_valid_queryparam(max_rut_query):
        qs = qs.filter(rut__lt=max_rut_query)    
...
def lista_personas(request):
    ...
    myFilter = OrderFilter(request.GET, queryset=persona)
    persona=myFilter.qs
	
    context = {'persona':persona,
	'myFilter':myFilter}
    return render(request, 'apppersona/lista_personas.html', {'lista': lista},'apppersona/filterpersonas.html',context)
